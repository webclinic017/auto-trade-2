...
mysql_host = config.db_mysql["host"]
mysql_port = config.db_mysql["port"]
mysql_user = config.db_mysql["user"]
mysql_pwd = config.db_mysql["password"]
mysql_db = config.db_mysql["db"]

print("mysql_host :", mysql_host, ",mysql_user :", mysql_user, ",mysql_db :", mysql_db)
url_mysql = config.get_url_mysql()
print("url_mysql :", url_mysql)

__version__ = "2.0.0"
# 每次发布时候更新。

def engine(url_db = url_mysql):
...
def engine_to_db(dbname):
    url  = config.get_url_mysql(dbname= dbname)
    engine = create_engine(
        url,
        encoding='utf8', convert_unicode=True)
    return engine
