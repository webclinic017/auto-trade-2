...
def submodel(x, iters, k_width, in_depth, out_depth):
  for i in range(iters):
    ...
    w = tf.Variable(tf.random.truncated_normal([k_width, k_width, cur_in_depth, out_depth], stddev=0.01))
    b = tf.Variable(tf.zeros([out_depth]))
    x = tf.nn.relu(tf.nn.conv2d(input=x, filters=w, strides=[1,1,1,1], padding='SAME') + b)
  x = tf.nn.max_pool2d(input=x, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
  return x

def fc(x, in_size, out_size):
  w = tf.Variable(tf.random.truncated_normal([in_size, out_size]))
...
def vgg16(x, filter_depth, hidden1, hidden2, img_height, img_width, img_depth, num_labels):
  ...
  out = tf.nn.dropout(out, rate=1 - (0.5))
