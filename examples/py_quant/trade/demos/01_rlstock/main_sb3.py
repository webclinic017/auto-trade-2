...
from envs import StockTradingEnv
...
def stock_trade(stock_file):
    ...
    # model = PPO2(MlpPolicy, env, verbose=0, tensorboard_log='./tmp/log') # SB2
    model = PPO("MlpPolicy", env, verbose=0, tensorboard_log='./tmp/log')
...
def test_a_stock_trade(stock_code):
    stock_file = find_file('./tmp/data/train', str(stock_code))
...
def multi_stock_trade():
    ...
    for code in range(start_code, start_code + max_num):
        stock_file = find_file('./tmp/data/train', str(code))
        if stock_file:
            try:
    ...
    with open(f'./tmp/code-{start_code}-{start_code + max_num}.pkl', 'wb') as f:
...
if __name__ == '__main__':
    ...
    # ret = find_file('./tmp/data/train', '600036')
    # print(ret)