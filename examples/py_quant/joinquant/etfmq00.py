...
def initialize(context):
    ...
    run_daily(ETFtrade2, time='14:59') #下单交易
...
def get_signal(context):
    ...
    for row in g.ETFList:
        security = row[0]
    # 获取指数的历史收盘价
        close_data = attribute_history(security, g.lag1 + 1, '1d', ['close'],df=False)
    # 获取指数当前收盘价
        # T日
        ...
        # T+1日
        current_price = close_data['close'][g.lag1]
        
    # 获取指数票的阶段收盘价涨幅百分比
        cp_increase = 100*(current_price/close_data['close'][0]-1)
    # 获取指数的收盘价
        close_data = attribute_history(security, g.lag2 + 1, '1d', ['close'],df=False)
    # 取得过去 g.lag2 天的平均价格
        # T日
        # ma_n1 = (current_price+close_data['close'].sum()-close_data['close'][0])/g.lag2
        # T+1日
        # ma_n1 = (close_data['close'].sum()-close_data['close'][g.lag2])/g.lag2
        ma_n1 = (close_data['close'].sum()-close_data['close'][0])/g.lag2
    # 计算最新收盘价与均线差值    
