...
# 2
# 不能使用 engine.execute() 但是，
# 因为 postgres 不允许在事务中创建数据库，
# 而 sqlalchemy 总是尝试在事务中运行查询。
# 要解决此问题，请从引擎获取底层连接
...
# 定时上传到ptrade的文件路径
g_ptrade_upload_path = './cache/a-share/data/file_update/trade_data.csv'

# ptrade输出交易数据文件目录
g_ptrade_export_dir = './cache/a-share/data/file_export/'
...
def create_pg_engine():
    ...
    #     connection.execute("CREATE DATABASE IF NOT EXISTS {0} ".format(dbname))
    # engine.dispose()
    ...
    if not database_exists(engine.url):
        print(f'not exit db {engine.url}')
        create_database(engine.url)
    # print(database_exists(engine.url))
    engine.dispose()
...
def create_mysql_engine():
    ...
    mysql_engine = sqlalchemy.create_engine(
        'mysql+pymysql://{0}:{1}@{2}:{3}'.format(user, password, host, port),
        poolclass=sqlalchemy.pool.NullPool
    )

    # 如果不存在数据库 db_quant 则创建
    mysql_engine.execute("CREATE DATABASE IF NOT EXISTS {0} ".format(dbname))

    # 创建连接数据库db_quant的引擎对象
    db_engine = sqlalchemy.create_engine(
        'mysql+pymysql://{0}:{1}@{2}:{3}/{4}?charset=utf8'.format(user, password, host, port, dbname),
...
def create_db_engine(use_mysql = False):
    if use_mysql:
...
def wrap_query_a_index_ak(date):
    """
    获取指数数据
    """
...
def wrap_query_a_summary_ak(date):
    """
    获取市场总览
    """
...
def wrap_query_a_all_stock_ak(date):
    """
    装饰股票代码查询函数
...
def wrap_query_a_all_stock_code_ak(date):
    """
    股票代码查询函数
...
def wrap_query_history_k_data_plus_ak(
    ...
    :param adjust: 复权类型，默认前复权：2 qfq；1 hfq：后复权；3：None 不复权
    :return: 包含K线数据的DataFrame
    """
    start_date = start_date.replace('-','', 2)
    ...
    df.columns = columns
    return df
    # return bs.query_history_k_data_plus(code, data_fields, start_date, end_date, frequency, adjust)    

def wrap_query_dividend_data_ak(code, year, yearType='report'):
    """
    装饰除权除息信息函数
...
def wrap_query_history_k_data_plus(code, data_fields=g_data_daily_fields_bs,
    ...
    :param end_date: 结束日期
    :param frequency: 数据类型，默认为d，日k线；d=日k线、w=周、m=月、5=5分钟、15=15分钟、30=30分钟、60=60分钟k线数据，不区分大小写；指数没有分钟线数据；周线每周最后一个交易日才可以获取，月线每月最后一个交易日才可以获取。
    :param adjust: 复权类型，默认前复权：2；1：后复权；3：不复权
    :return: 包含K线数据的DataFrame
    """

    return bs.query_history_k_data_plus(code, data_fields, start_date, end_date, frequency, adjustflag=adjust)
...
def get_stock_codes(date=None, update=False):
    ...
    if table_name not in sqlalchemy.inspect(engine).get_table_names() or update:
        ...
        # 获取最新交易日
        trading_date = None if True else get_trading_date(date)

        # 查询股票数据
        stock_df = wrap_query_a_all_stock_code_ak(trading_date)
...
def update_data(code, latest_trading_date, db_tables, query_days=60, adjust='qfq', recreate=False):
    ...
    :param adjust: 复权选项 1 hfq：后复权 2 qfq：前复权  None：不复权  默认为前复权
    ...
    if False:
        convert_list = ['open', 'high', 'low', 'close', 'volume', 'amount', 'turn', 'pctChg']
...
def zt(df):
    ...
    df['zt'] = np.where((df['close'].values >= 1.098 * df['close'].shift(1).values), True, False)
...
def profit_loss_statistic(code, hold_days=10):
    ...
    table_name = f'stock_a_history_1d_{code}'
    # 计算收益分布循环
    print(f'收益分析 正在处理 {code} table name: {table_name}...')
