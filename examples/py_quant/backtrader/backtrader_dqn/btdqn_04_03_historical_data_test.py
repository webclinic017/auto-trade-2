...
import numpy as np
from market_env import MarketEnv87
        ...
        while not game_over:
            ...
            if env.actions[action] == "LONG" or env.actions[action] == "SHORT":
                ...
                if isRandom:
                    ...
                    print("%s:\t%s\t%.2f\t%.2f\t" % \
                        (info["dt"], \
                        color + env.actions[action] + bcolors.ENDC, \
                        cumReward, \
                        info["cum"]) + ("\t".join(["%s:%.2f" % (l, i) for l, i in zip(env.actions, q[0].tolist())]) if isRandom == False else "")
                    )
