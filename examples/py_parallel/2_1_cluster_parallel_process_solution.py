...
# -*- coding: utf-8 -*-
"""
Created on Mon Jul 26 10:58:29 2022
...
dbname = 'stock_data_for_test';
dbname2 = 'stock_data';

#  创建数据库 股票分析数据库 的连接 
conn, curs, engine = ppp.creat_conn_with_pg(dbname = dbname)

#  创建数据库 stock_data 的连接 
conn2, curs2, engine2 = ppp.creat_conn_with_pg(dbname = dbname2)
...
# 如果是中断后执行，则仅执行以下操作：将处理过程中的处理记录归零
curs.execute("""UPDATE      parallel_tool_并行任务分配信息表
                    SET     处理状态     = %s
                    WHERE   处理状态     = 1           -- 处理状态为 1，表示正在处理过程中...(这类分组的状态要归零，因为中断前未处理完毕)  
                    ;""", 
               (0,
                ))
conn.commit()
...
def foo(i):
    ...
    while True:
        ...
        # 给出需要导入数据的“日期”：随机取一个（随机取：以确保）
        date_list = list(df['交易日期'])
        ...
        if count_bat%50 == 1:
            ...
            print('代码文件 2_1_集群版_分布式并行计算方案_*.py 的第2步-->>> 第 {} 个子进程工作进度：已完成第 {} 组的数据处理，还有 {} 组数据等待处理 '.format(i, count_bat, len(df)))
