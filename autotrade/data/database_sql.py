...
from autotrade.common.utils import get_file_path
...
def init_models(db: Database, driver: Driver):
    class DbBarEntity(ModelBase):
        ...
        @staticmethod
        def from_bar(bar: BarEntity):
            """
            Generate DbBarEntity object from BarEntity.
            """
            db_bar = DbBarEntity()
        ...
        def to_bar(self):
            """
            Generate BarEntity object from DbBarEntity.
            """
            bar = BarEntity(
                symbol=self.symbol,
        ...
        @staticmethod
        def save_all(objs: List["DbBarEntity"]):
            """
            ...
            with db.atomic():
                if driver is Driver.POSTGRESQL:
                    for bar in dicts:
                        DbBarEntity.insert(bar).on_conflict(
                            update=bar,
                            conflict_target=(
                                DbBarEntity.datetime,
                                DbBarEntity.interval,
                                DbBarEntity.symbol,
                                DbBarEntity.exchange,
                            ),
                        ).execute()
                else:
                    for c in chunked(dicts, 50):
                        DbBarEntity.insert_many(
                            c).on_conflict_replace().execute()

    class DbTickEntity(ModelBase):
        """
        ...
        @staticmethod
        def from_tick(tick: TickEntity):
            """
            Generate DbTickEntity object from TickEntity.
            """
            db_tick = DbTickEntity()
        ...
        def to_tick(self):
            """
            Generate TickEntity object from DbTickEntity.
            """
            tick = TickEntity(
                symbol=self.symbol,
                exchange=Exchange(self.exchange),
        ...
        @staticmethod
        def save_all(objs: List["DbTickEntity"]):
            dicts = [i.to_dict() for i in objs]
            with db.atomic():
                if driver is Driver.POSTGRESQL:
                    for tick in dicts:
                        DbTickEntity.insert(tick).on_conflict(
                            update=tick,
                            conflict_target=(
                                DbTickEntity.datetime,
                                DbTickEntity.symbol,
                                DbTickEntity.exchange,
                            ),
                        ).execute()
                else:
                    for c in chunked(dicts, 50):
                        DbTickEntity.insert_many(
                            c).on_conflict_replace().execute()

    db.connect()
    db.create_tables([DbBarEntity, DbTickEntity])
    return DbBarEntity, DbTickEntity
...
class SqlManager(BaseDatabaseManager):
    ...
    def save_bar_data(self, datas: Sequence[BarEntity]):
        ds = [self.class_bar.from_bar(i) for i in datas]
        self.class_bar.save_all(ds)

    def save_tick_data(self, datas: Sequence[TickEntity]):
